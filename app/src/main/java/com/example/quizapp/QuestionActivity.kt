...
class QuestionActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        submit.setOnClickListener {
            if (selectedOption!=0){
                val question=questionList!![currentPositon-1]
                ...
                if (currentPositon==questionList!!.size)
                    submit.text="FINISH"
                else
                ...
                currentPositon++
                when{
                    currentPositon<=questionList!!.size->{
                        setQuestion()
                    }
                    else->{
    ...
    fun setQuestion(){
        ...
        progress_bar.progress=currentPositon
        progress_bar.max=questionList!!.size
        progress_text.text = "${currentPositon}"+"/"+"${questionList!!.size}"
